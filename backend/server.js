...
// Checagem de saude (para o Easypanel/monitoramento). Sem login.
...
// Respostas rapidas (templates), sob /api/templates
...
// Handler de erro global: evita derrubar o processo em erros sincronos das rotas.
...
app.listen(PORT, () => {
  ...
  // Primeira reconciliacao logo apos subir (antes so acontecia depois de
  // 10 min, deixando o painel vazio no boot).
  reconcileAllAccounts().catch((err) =>
    console.error("[reconcile-boot] erro:", err.message)
  );
});
